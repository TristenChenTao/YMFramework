...
//Vendor Modified by YM:self.hudView.bounds修改
//self.hudView.bounds = CGRectMake(0.0f, 0.0f, hudWidth, hudHeight);
    if (kYm_iPhone6Plus) {
        self.hudView.bounds = CGRectMake(0, 0, 168, 168);
    }
    else {
        self.hudView.bounds = CGRectMake(0, 0, 130, 130);
    }
    
    if(string) {
//Vendor Modified by YM:self.imageView 位置修改
//self.imageView.center = CGPointMake(CGRectGetWidth(self.hudView.bounds)/2, 36.0f);
    if (kYm_iPhone6Plus) {
        self.imageView.center = CGPointMake(CGRectGetWidth(self.hudView.bounds)/2, 60.0f);
    }
    else {
        self.imageView.center = CGPointMake(CGRectGetWidth(self.hudView.bounds)/2, 46.0f);
    }
 }
	 else
...
//Vendor Modified by YM:self.stringLabel 位置修改
//self.stringLabel.frame = labelRect;
    if (kYm_iPhone6Plus) {
        self.stringLabel.frame = CGRectMake(0, self.distance, 168, stringHeight);
    }
    else {
        self.stringLabel.frame = CGRectMake(0, self.distance, 130, stringHeight);
    }
...
//Vendor Modified by YM:indefiniteAnimatedView 位置修改
//CGPoint center = CGPointMake((CGRectGetWidth(self.hudView.bounds)/2), 36.0f);
       CGPoint center = CGPointZero;
      if (kYm_iPhone6Plus) {
          center = CGPointMake(CGRectGetWidth(self.hudView.bounds)/2, 60.0f);
      }
      else {
          center = CGPointMake(CGRectGetWidth(self.hudView.bounds)/2, 46.0f);
      }
...
- (void)showImage:(UIImage *)image status:(NSString *)string duration:(NSTimeInterval)duration maskType:(SVProgressHUDMaskType)hudMaskType {
    ...
    //Vendor Modified by YM:imageView 修改大小
...
+ (void)setTextToTop:(CGFloat)distance
{
    [self sharedView].distance = distance;
}
